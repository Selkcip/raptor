...
public class DecalSystem : MonoBehaviour {
	...
	private static DecalSystem s_instance = null;
	...
	[SerializeField]
	private DS_Decals m_DecalsPrefab = null;
	...
	// create at runtime.
	[SerializeField]
	private Vector3 m_DecalProjectorScale = new Vector3(0.2f, 2.0f, 0.2f);
	[SerializeField]
	private float m_CullingAngle = 80.0f;
	[SerializeField]
	private float m_MeshOffset = 0.0f;
	...
	public void CreateDecal(Vector3 point, Vector3 dir, Collider collider, int type = 0) {
		type = type % m_DecalsInstance.CurrentUvRectangles.Length;
		if(collider.rigidbody != null) {
			DecaledObject decaledObject = collider.GetComponent<DecaledObject>();
			if(decaledObject != null) {
				decaledObject.AddDecalProjector(point, dir, collider); //Add type
			}
		}
		else {
			// Collider hit.
			...
			Vector3 l_ProjectorPosition = point -(m_DecalProjectorOffset * dir.normalized);
			Quaternion l_ProjectorRotation = ProjectorRotationUtility.ProjectorRotation(dir.normalized, Vector3.up);
			...
			MeshCollider l_MeshCollider = collider.GetComponent<MeshCollider>();
			MeshFilter l_MeshFilter = collider.GetComponent<MeshFilter>();
			...
			if(l_MeshCollider != null || l_MeshFilter != null) {
				...
				if(l_Mesh != null) {
					...
					DecalProjector l_DecalProjector = new DecalProjector(l_ProjectorPosition, l_ProjectorRotation, m_DecalProjectorScale, m_CullingAngle, m_MeshOffset, type, type);
					...
					Matrix4x4 l_WorldToMeshMatrix = collider.renderer.transform.worldToLocalMatrix;
		...
		if(Input.GetButtonDown("Fire1")) {
			...
			if(Physics.Raycast(l_Ray, out l_RaycastHit, Mathf.Infinity)) {
				...
				if(l_RaycastHit.rigidbody != null) {
					...
					DecaledObject decaledObject = l_RaycastHit.collider.GetComponent<DecaledObject>();
					if(decaledObject != null) {
						//decaledObject.AddDecalProjector(l_Ray, l_RaycastHit);
					}
				}
				else {
					//CreateDecal(l_RaycastHit.point, l_Ray.direction, l_RaycastHit.collider);
					/*// Collider hit.
