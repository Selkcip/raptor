...
public class FirstPersonHeadBob : MonoBehaviour, IEventListener {
	...
	[SerializeField] float headBobFrequency = 1.0f;				// the base speed of the head bobbing (in cycles per metre)
	[SerializeField] float headBobHeight = 0.3f;				// the height range of the head bob
	[SerializeField] float headBobSwayAngle = 0.2f;				// the angle which the head tilts to left & right during the bob cycle
	[SerializeField] float headBobSideMovement = 0.01f;			// the distance the head moves to left & right during the bob cycle
	...
	[SerializeField] float jumpLandMove = 1.5f;
	[SerializeField] float jumpLandTilt = 10;
	...
	float springElastic = 1.8f;		
	float springDampen = 0.6f;
	float springVelocityThreshold = 0.001f;
	float springPositionThreshold = 0.001f;
    ...
    bool portalTransition = false;


	// Use this for initialization
	void Start () {
        // Register as Event Listener
        EventManager.instance.AddListener(this, "EnteredPortalEvent", PortalTransition);
    ...
    //Required to be a IEventListener
    public bool HandleEvent(IEvent evt) {
        return false;
    }

    //Function that fixes the portal transition effect.
    public bool PortalTransition(IEvent evt) {
