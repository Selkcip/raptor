...
public class UnitySerializerWindow : EditorWindow
{
	...
	static UnitySerializerWindow ()
	{
		...
		room = AssetDatabase.LoadAssetAtPath(path + "SaveRoom.psd", typeof(Texture2D)) as Texture2D;
	...
	void OnGUI ()
	{
		...
		if (showRoom) 
		{
			if (GameObject.FindObjectsOfType (typeof(SaveRoom)).Length > 0) {
			...
			if (GameObject.FindObjectsOfType (typeof(SaveRoom)).Length == 0) {
				...
				var style = new GUIStyle ("label");
				style.wordWrap = true;
				GUILayout.Label ("You need to assign a SaveRoom Manager script to an object in this scene.  This object should be something that IS loaded and saved with one of the scripts, and NOT set to DontDestroyOnLoad.", style);
				GUILayout.Space (6);
				GUI.color = Color.white;
				using (new Horizontal()) {
					if (GUILayout.Button ("Create a new SaveRoom Manager")) {
						Undo.RegisterSceneUndo ("Create room manager");
						var go = new GameObject ();
						go.name = "SaveRoom Manager";
						go.AddComponent<StoreInformation> ();
						go.AddComponent<RoomDataSaveGameStorage>();
						go.AddComponent<SaveRoom>();
					}
				}
				GUILayout.Space (15);
			} 
