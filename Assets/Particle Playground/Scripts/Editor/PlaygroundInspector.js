...
class PlaygroundInspector extends Editor {
	...
	function OnSceneGUI () {
		RenderManipulatorsInScene();
		if (GUI.changed)
            EditorUtility.SetDirty (target);
	}
	
	static function RenderManipulatorsInScene () {
		...
		if (playgroundScriptReference!=null && playgroundScriptReference.drawGizmos && manipulatorsFoldout) {
			...
			for (var i = 0; i<playgroundScriptReference.manipulators.Count; i++) {
				if (playgroundScriptReference.manipulators[i].transform) {
					Handles.color = Color(.4,.6,1.0,Mathf.Clamp(Mathf.Abs(playgroundScriptReference.manipulators[i].strength),.25,1.0));
					Handles.color = playgroundScriptReference.manipulators[i].enabled? Handles.color : Color(.4,.6,1.0,.2);
					
					// Position
					if (Tools.current==Tool.Move)
						playgroundScriptReference.manipulators[i].transform.position = Handles.PositionHandle(playgroundScriptReference.manipulators[i].transform.position, Tools.pivotRotation==PivotRotation.Global? Quaternion.identity : playgroundScriptReference.manipulators[i].transform.rotation);
					// Rotation
					else if (Tools.current==Tool.Rotate)
						playgroundScriptReference.manipulators[i].transform.rotation = Handles.RotationHandle(playgroundScriptReference.manipulators[i].transform.rotation, playgroundScriptReference.manipulators[i].transform.position);
					// Scale
					else if (Tools.current==Tool.Scale)
						playgroundScriptReference.manipulators[i].transform.localScale = Handles.ScaleHandle(playgroundScriptReference.manipulators[i].transform.localScale, playgroundScriptReference.manipulators[i].transform.position, playgroundScriptReference.manipulators[i].transform.rotation, HandleUtility.GetHandleSize(playgroundScriptReference.manipulators[i].transform.position));
					
					id = GUIUtility.GetControlID(FocusType.Passive);
					
					// Sphere Size
					if (playgroundScriptReference.manipulators[i].shape==MANIPULATORSHAPE.Sphere) {
						playgroundScriptReference.manipulators[i].size = Handles.RadiusHandle (Quaternion.identity, playgroundScriptReference.manipulators[i].transform.position, playgroundScriptReference.manipulators[i].size);
						if (playgroundScriptReference.manipulators[i].enabled && GUIUtility.hotControl>0)
							Handles.Label(playgroundScriptReference.manipulators[i].transform.position+Vector3(playgroundScriptReference.manipulators[i].size+1,1,0), "Size: "+playgroundScriptReference.manipulators[i].size.ToString("f2"));
					
					// Box Bounds
					} else {
						DrawManipulatorBox(playgroundScriptReference.manipulators[i]);
					}
					
					// Strength
					manipulatorHandlePosition = playgroundScriptReference.manipulators[i].transform.position+Vector3(0,playgroundScriptReference.manipulators[i].strength,0);
					
					Handles.DrawLine(playgroundScriptReference.manipulators[i].transform.position, manipulatorHandlePosition);
					id = GUIUtility.GetControlID(FocusType.Passive);
					playgroundScriptReference.manipulators[i].strength = Handles.ScaleValueHandle(playgroundScriptReference.manipulators[i].strength, manipulatorHandlePosition, Quaternion.identity, HandleUtility.GetHandleSize(manipulatorHandlePosition), Handles.SphereCap, 1);      
					if (playgroundScriptReference.manipulators[i].enabled && GUIUtility.hotControl>0)
						Handles.Label(manipulatorHandlePosition+Vector3(1,1,0), "Strength: "+playgroundScriptReference.manipulators[i].strength.ToString("f2"));
					
					Handles.color = Color(.4,.6,1.0,.025);
					Handles.DrawSolidDisc(playgroundScriptReference.manipulators[i].transform.position, Camera.current.transform.forward, playgroundScriptReference.manipulators[i].strength);
					Handles.color = Color(.4,.6,1.0,.5);
					Handles.DrawSolidDisc(playgroundScriptReference.manipulators[i].transform.position, Camera.current.transform.forward, HandleUtility.GetHandleSize(playgroundScriptReference.manipulators[i].transform.position)*.05);
				}
			}
		}
	}
	...
	static function RenderPlaygroundSettings () {
		...
		if (playgroundFoldout && playgroundScriptReference!=null) {
			...
			// Manipulators
			if (GUILayout.Button("Manipulators ("+playgroundScriptReference.manipulators.Count+")", EditorStyles.toolbarDropDown)) manipulatorsFoldout=!manipulatorsFoldout;
			if (manipulatorsFoldout) {
				...
				if (manipulators.arraySize>0) {
					...
					var thisManipulator : SerializedProperty;
					var thisManipulatorAffects : SerializedProperty;
					var thisManipulatorSize : SerializedProperty;
					var thisManipulatorStrength : SerializedProperty;
					for (var i = 0; i<manipulators.arraySize; i++) {
					
						thisManipulator = manipulators.GetArrayElementAtIndex(i);
						thisManipulatorAffects = thisManipulator.FindPropertyRelative("affects");
						...
						if (manipulatorListFoldout[i]) {
							playgroundScriptReference.manipulators[i].enabled = EditorGUILayout.ToggleLeft("Enabled", playgroundScriptReference.manipulators[i].enabled);
							EditorGUI.indentLevel++;
							GUI.enabled = playgroundScriptReference.manipulators[i].enabled;
							if (i<manipulators.arraySize) {
								EditorGUILayout.PropertyField(thisManipulator.FindPropertyRelative("transform"), new GUIContent("Transform"));
								if (playgroundScriptReference.manipulators[i].transform) {
									EditorGUI.indentLevel++;
									playgroundScriptReference.manipulators[i].transform.position = EditorGUILayout.Vector3Field("Position", playgroundScriptReference.manipulators[i].transform.position);
									playgroundScriptReference.manipulators[i].transform.rotation = Quaternion.Euler(EditorGUILayout.Vector3Field("Rotation", playgroundScriptReference.manipulators[i].transform.rotation.eulerAngles));
									playgroundScriptReference.manipulators[i].transform.localScale = EditorGUILayout.Vector3Field("Scale", playgroundScriptReference.manipulators[i].transform.localScale);
									EditorGUI.indentLevel--;
								}
								EditorGUILayout.Separator();
								EditorGUILayout.PropertyField(thisManipulator.FindPropertyRelative("type"), new GUIContent("Type"));
								
								// Render properties
								if (playgroundScriptReference.manipulators[i].type==MANIPULATORTYPE.Property) {
									if (!playgroundScriptReference.manipulators[i].property) playgroundScriptReference.manipulators[i].property = new ManipulatorProperty();
									
									playgroundScriptReference.manipulators[i].property.type = System.Convert.ToInt32(EditorGUILayout.EnumPopup("Property Type", playgroundScriptReference.manipulators[i].property.type));
									
									// Velocity
									if (playgroundScriptReference.manipulators[i].property.type==MANIPULATORPROPERTYTYPE.Velocity || playgroundScriptReference.manipulators[i].property.type==MANIPULATORPROPERTYTYPE.AdditiveVelocity) {
										playgroundScriptReference.manipulators[i].property.velocity = EditorGUILayout.Vector3Field("Property Velocity", playgroundScriptReference.manipulators[i].property.velocity);
										playgroundScriptReference.manipulators[i].property.useLocalRotation = EditorGUILayout.Toggle("Local Rotation", playgroundScriptReference.manipulators[i].property.useLocalRotation);
									} else 
									// Color
									if (playgroundScriptReference.manipulators[i].property.type==MANIPULATORPROPERTYTYPE.Color) {
										playgroundScriptReference.manipulators[i].property.color = EditorGUILayout.ColorField("Property Color", playgroundScriptReference.manipulators[i].property.color);
										playgroundScriptReference.manipulators[i].property.onlyColorInRange = EditorGUILayout.Toggle("Only Color In Range", playgroundScriptReference.manipulators[i].property.onlyColorInRange);
										playgroundScriptReference.manipulators[i].property.keepColorAlphas = EditorGUILayout.Toggle("Keep Color Alphas", playgroundScriptReference.manipulators[i].property.keepColorAlphas);
									} else 
									if (playgroundScriptReference.manipulators[i].property.type==MANIPULATORPROPERTYTYPE.Size) {
										playgroundScriptReference.manipulators[i].property.size = EditorGUILayout.FloatField("Property Size", playgroundScriptReference.manipulators[i].property.size);
									}
									
									// Transition
									if (playgroundScriptReference.manipulators[i].property.type!=MANIPULATORPROPERTYTYPE.None) {
										playgroundScriptReference.manipulators[i].property.transition = System.Convert.ToInt32(EditorGUILayout.EnumPopup("Transition", playgroundScriptReference.manipulators[i].property.transition));
									}
								}
								EditorGUILayout.Separator();
								EditorGUILayout.PropertyField(thisManipulator.FindPropertyRelative("shape"), new GUIContent("Shape"));
								
								if (playgroundScriptReference.manipulators[i].shape==MANIPULATORSHAPE.Sphere) {
									thisManipulatorSize = thisManipulator.FindPropertyRelative("size");
									thisManipulatorSize.floatValue = EditorGUILayout.Slider("Size", thisManipulatorSize.floatValue, 0, maximumAllowedManipulatorSize);
								} else {
									EditorGUILayout.PropertyField(thisManipulator.FindPropertyRelative("bounds"), new GUIContent("Bounds"));
								}
								EditorGUILayout.Separator();
								thisManipulatorStrength = thisManipulator.FindPropertyRelative("strength");
								thisManipulatorStrength.floatValue = EditorGUILayout.Slider("Strength", thisManipulatorStrength.floatValue, 0, maximumAllowedManipulatorStrength);
								EditorGUILayout.Separator();
								EditorGUILayout.PropertyField(thisManipulatorAffects, new GUIContent("Affects"));
							}
							EditorGUI.indentLevel--;
						}
						GUI.enabled = true;
