...
class PlaygroundParticleWindow extends EditorWindow {
	...
	// Paths
	static var presetPath : String = "Assets/Particle Playground/Resources/Presets/";
	static var iconPath : String = "Assets/Particle Playground/Graphics/Editor/Icons/";
	static var brushPath : String = "Assets/Particle Playground/Resources/Brushes/";
	...
	@MenuItem ("Window/Playground Wizard")
	...
	function Initialize () {
		...
		var particlePrefabs : Object[] = Resources.LoadAll("Presets", PlaygroundParticles);
		var particleImageDefault : Texture2D = Resources.LoadAssetAtPath(iconPath+"Default.png", Texture2D) as Texture2D;
		...
		for (var i = 0; i<particlePrefabs.Length; i++) {
			...
			particleImage = Resources.LoadAssetAtPath(iconPath+presetObjects[i].presetObject.name+".png", Texture2D) as Texture2D;
	...
	function OnGUI () {
		...
		EditorGUILayout.LabelField("Playground Wizard", EditorStyles.largeLabel, GUILayout.Height(20));
		EditorGUILayout.Separator();
		EditorGUILayout.BeginVertical("box");
		...
		// Presets
		EditorGUILayout.BeginVertical("box");
		if (GUILayout.Button((particlePresetFoldout?"▼":"►")+" Presets ", EditorStyles.label)) particlePresetFoldout=!particlePresetFoldout;
			...
			if (presetObjects.Count>0) {
				...
				for (var i = 0; i<presetObjects.Count; i++) {
					...
					if (presetListStyle==1) {
						...
						EditorGUILayout.BeginHorizontal();
						if (GUILayout.Button((presetObjects[i].foldout?"▼ ":"► ")+presetObjects[i].presetObject.name, EditorStyles.label)) presetObjects[i].foldout=!presetObjects[i].foldout;
						EditorGUILayout.Separator();
				...
				EditorGUILayout.HelpBox("No presets found. Make sure that the path to the presets are set to: \""+presetPath+"\"", MessageType.Info);
			}
