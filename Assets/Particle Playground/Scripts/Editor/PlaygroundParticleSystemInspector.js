...
class PlaygroundParticleSystemInspector extends Editor {
	...
	static var calculateDeltaMovement : SerializedProperty;		// boolean
	...
	static var gravity : SerializedProperty;					// Vector3
	static var damping : SerializedProperty;					// float
	...
	static var calculateDeltaMovementSetting : boolean;
	...
	function OnEnable () {
		...
		calculateDeltaMovement = playgroundParticles.FindProperty("calculateDeltaMovement");
		...
		gravity = playgroundParticles.FindProperty("gravity");
		damping = playgroundParticles.FindProperty("damping");
		...
		calculateDeltaMovementSetting = playgroundParticlesScriptReference.calculateDeltaMovement;
	...
	function OnDestroy () {
		...
		playgroundParticlesScriptReference.calculateDeltaMovement = calculateDeltaMovementSetting;
	}
	...
	override function OnInspectorGUI () {
		...
		if (particlesFoldout) {
			...
			if (sourceFoldout) {
				...
				var previousSource : int = source.intValue;
				...
				// Source is Script
					...
					EditorGUILayout.Separator();
					EditorGUILayout.HelpBox("This Particle Playground System is controlled by script. You can only create new particle emission positions from script when using this source mode. This is an advanced feature and should not be confused with altering a Particle Playground System through script (which is available for all source modes). Please refer to the manual and script reference for detailed information.", MessageType.Info);
			...
			if (particleSettingsFoldout) {
				...
				emit.boolValue = EditorGUILayout.Toggle("Emit Particles", emit.boolValue);
				...
				if (playgroundParticlesScriptReference.source==SOURCE.State && playgroundParticlesScriptReference.states!=null && playgroundParticlesScriptReference.states.Count>0 && playgroundParticlesScriptReference.states[playgroundParticlesScriptReference.activeState].stateTransform==null) {
					...
					calculateDeltaMovement.boolValue = EditorGUILayout.ToggleLeft("Calculate Delta Movement", calculateDeltaMovement.boolValue);
				GUI.enabled = (GUI.enabled && calculateDeltaMovement.boolValue);
					EditorGUI.indentLevel++;
			...
			if (forcesFoldout) {
				...
				if (playgroundParticlesScriptReference.source==SOURCE.State && playgroundParticlesScriptReference.states!=null && playgroundParticlesScriptReference.states.Count>0 && playgroundParticlesScriptReference.states[playgroundParticlesScriptReference.activeState].stateTransform==null) {
					...
					GUI.enabled = false;
				} else GUI.enabled = (applyInitialLocalVelocity.boolValue&&!onlySourcePositioning.boolValue);
				...
				gravity.vector3Value = EditorGUILayout.Vector3Field("Gravity", gravity.vector3Value);
				damping.floatValue = EditorGUILayout.Slider("Damping", damping.floatValue, 0, maximumAllowedDamping);
		...
		if (statesFoldout) {
			if (states.arraySize>0) {
				...
				for (var i = 0; i<states.arraySize; i++) {
					...
					if (playgroundParticlesScriptReference.activeState==i) GUILayout.BeginHorizontal("box");
	...
	function StartStopPaint () {
		...
		if (inPaintMode) {
			calculateDeltaMovementSetting = playgroundParticlesScriptReference.calculateDeltaMovement;
			playgroundParticlesScriptReference.calculateDeltaMovement = false;
	...
	function OnSceneGUI () {
		...
		PlaygroundInspector.RenderManipulatorsInScene();
		if (GUI.changed)
