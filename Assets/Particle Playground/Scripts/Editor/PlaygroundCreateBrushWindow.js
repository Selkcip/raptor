...
class PlaygroundCreateBrushWindow extends EditorWindow {
	...
	function OnGUI () {
		...
		if(GUILayout.Button("Create", EditorStyles.toolbarButton, GUILayout.Width(50))){
			...
			if (brushTexture && brushName!="") {
				if (AssetDatabase.LoadAssetAtPath(PlaygroundParticleWindow.brushPath+brushName+".prefab", GameObject)) {
	...
	function CreateBrush () {
		...
		var brushPrefab : GameObject = PrefabUtility.CreatePrefab(PlaygroundParticleWindow.brushPath+brushName+".prefab", brushObject, ReplacePrefabOptions.Default);
