...
class Playground extends MonoBehaviour {
	...
	static var collisionSleepVelocity : float = .1;				// Minimum velocity of a particle before it goes to rest
	
	// Paint settings
	var paintMaxPositions : int = 10000;						// Maximum amount of emission positions in a PaintObject
	...
	var calculate : boolean = true;								// Calculate forces on PlaygroundParticles objects
	var pixelFilterMode : PIXELMODE = PIXELMODE.Pixel32;		// Color filtering mode
	var garbageCollectOnResize : boolean = true;				// Issue a GC.Collect when particle lists are resized
	var autoGroup : boolean = true;								// Automatically parent a PlaygroundParticles object to Playground if it has no parent
 	var buildZeroAlphaPixels : boolean = false;					// Turn this on if you want to build particles from 0 alpha pixels into states
 	var drawGizmos : boolean = true;							// Draw gizmos for manipulators in Scene View
 	var paintToolbox : boolean = true;							// Show toolbox in Scene View when Source is set to Paint on PlaygroundParticles objects
	...
	static var version : String = "1.125";
	...
	static function ManipulatorObject (type:MANIPULATORTYPE, affects:LayerMask, manipulatorTransform:Transform, size:float, strength:float) : ManipulatorObject {
		return PlaygroundParticles.NewManipulatorObject(type,affects,manipulatorTransform,size,strength);
	}
	...
	static function ManipulatorObject (manipulatorTransform:Transform) : ManipulatorObject {
		...
		return PlaygroundParticles.NewManipulatorObject(MANIPULATORTYPE.Attractor,layerMask,manipulatorTransform,1.0,1.0);
	}
	...
	static function Paint (playgroundParticles:PlaygroundParticles, position:Vector3, normal:Vector3, parent:Transform, color:Color32) {
		playgroundParticles.isPainting = true;
		playgroundParticles.paint.Paint(position,normal,parent,color);
	}
	...
	static function Emission (playgroundParticles:PlaygroundParticles, emit:boolean) {
		PlaygroundParticles.Emission(playgroundParticles,emit);
	}
	...
	static function InstantiatePreset (presetName : String) : PlaygroundParticles {
		...
		if (presetParticles!=null) {
			if (reference==null) {
				...
				ResourceInstantiate("Playground Manager");
			}
	...
	static function Offset (origin : ORIGIN, imageWidth : int, imageHeight : int, meshScale : float) : Vector3 {
		var offset : Vector3 = Vector3.zero;
		switch (origin) {
			case ORIGIN.TopLeft: 		offset.y = -imageHeight*meshScale; break;
			case ORIGIN.TopCenter: 		offset.y = -imageHeight*meshScale; offset.x = (-imageWidth*meshScale)/2; break;
			case ORIGIN.TopRight: 		offset.y = -imageHeight*meshScale; offset.x = (-imageWidth*meshScale); break;
			case ORIGIN.MiddleLeft: 	offset.y = -imageHeight*meshScale/2; break;
			case ORIGIN.MiddleCenter:	offset.y = -imageHeight*meshScale/2; offset.x = (-imageWidth*meshScale)/2; break;
			case ORIGIN.MiddleRight:	offset.y = -imageHeight*meshScale/2; offset.x = (-imageWidth*meshScale); break;
			case ORIGIN.BottomCenter:	offset.x = (-imageWidth*meshScale)/2; break;
			case ORIGIN.BottomRight:	offset.x = (-imageWidth*meshScale); break;
		}
		return offset;
	}
	...
	static function ResourceInstantiate (n : String) : GameObject {
		var go : GameObject = Instantiate(UnityEngine.Resources.Load(n) as GameObject);
		go.name = n;
		return go;
	}
	...
	function InitializePlayground () {
		...
		for (var i = 0; i<particleSystems.Count; i++) {
			if (particleSystems[i]!=null) {
				...
				if (particleSystems[i].states.Count>0) {
					for (var x = 0; x<particleSystems[i].states.Count; x++) {
						particleSystems[i].states[x].Initialize();
					}
				}
			} else {
		...
		// Set particle count (prepares all necessary arrays)
		for (i = 0; i<particleSystems.Count; i++) {
			Playground.SetParticleCount(particleSystems[i], particleSystems[i].particleCount);
			particleSystems[i].YieldedRefresh();
		}
	...
	function UpdateRoutine () {
		...
		#if UNITY_EDITOR
		while (EditorApplication.isPlaying && reference.enabled) {
			PlaygroundUpdate();
			yield;
		}
		#else
		while (reference.enabled) {
			PlaygroundUpdate();
			yield;
		}
		#endif
	...
	function PlaygroundUpdate () {
		...
		for (var i = 0; i<particleSystems.Count; i++) {
			...
			particleSystems[i].source!=SOURCE.Script && 
			particleSystems[i].particleCache!=null &&
	...
	function OnEnable () {
		InitializePlayground();
       		...
       		var callBackUpdate : EditorApplication.CallbackFunction = PlaygroundUpdate;
       		EditorApplication.update = Delegate.Combine(callBackUpdate, EditorApplication.update) as EditorApplication.CallbackFunction;
       	#endif
	}
	...
	function OnDisable () {
			...
			if (reference!=this) return;
			var callBackUpdate : EditorApplication.CallbackFunction = PlaygroundUpdate;
			EditorApplication.update = Delegate.Remove(callBackUpdate, EditorApplication.update) as EditorApplication.CallbackFunction;
		#endif
	}
...
class PaintObject {
	...
	// Live paint into this PaintObject (single point with information about position, normal, parent and color)
	function Paint (pos : Vector3, norm : Vector3, parent : Transform, color : Color32) {		
	...
	function ConstructParticles (image : Texture2D, scale : float, offset : Vector3, newStateName : String, newStateTransform : Transform) {
		...
		for (var i = 0; i<color.Length; i++) {
			if (readAlpha || color[i].a!=0) {
				...
				tmpPosition.Add(Vector3(x,y,0)+offset);			
			}
			x++; x=x%image.width;
			if (x==0 && i!=0) y++;
		}
...
class WorldObjectBase {
	...
	@HideInInspector var meshFilter : MeshFilter;		// The mesh filter of this World Object
...
class ManipulatorObject {
	...
	function Contains (position : Vector3) : boolean {
		if (shape==MANIPULATORSHAPE.Box) {
			return bounds.Contains(transform.InverseTransformPoint(position));
		} else {
			// This is never reached from the Calculate-loop as distance is still needed as variable internally
			if (Vector3.Distance(position, transform.position)<size) 
				return true;
		}
		return false;
	}
