...
// Cached components
private var particles : PlaygroundParticles;				// The cached script of our Playground Particle System (also exposed through Playground.reference.particleSystems[])
...
function OnGUI () {
	...
	if (showGui) {
		...
		if (manipulator.enabled) {
			...
			if (GUI.changed) {
				manipulatorGuiSizeTransform.localScale = Vector3(manipulator.size, manipulator.size, manipulator.size)*1.1;
		...
		switch (selectedSource) {
			...
			break;
			case 1:
				particles.skinnedWorldObject.renderer.enabled = GUILayout.Toggle(particles.skinnedWorldObject.renderer.enabled, "Render Mesh");
			break;
			case 2:
...
function LiftRigidbody () {
	...
	if (Physics.Raycast(ray, hit, 100, interactionLayer) && hit.rigidbody) {
		...
		//springJoint.anchor = trans.InverseTransformPoint(hit.point);
